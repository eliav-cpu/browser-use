...
זה אומר שהממשק שבו אתה נמצא לא מאפשר לפתוח או להציג קובץ ZIP בינארי. לכן כל קבצי המערכת נשמרים גם כקבצי טקסט רגילים (`.csv`, `.md`, `.json`, `.gs`) שאפשר לפתוח, להעתיק או להוריד אחד-אחד.
